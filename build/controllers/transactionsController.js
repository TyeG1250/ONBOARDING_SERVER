...
import { Transaction, AddGatewayV1, AssertLocationV1, AssertLocationV2 } from '@helium/transactions';
...
export const pay = async (req, res) => {
  try {
    ...
    switch (Transaction.stringType(transaction)) {
      ...
      case 'assertLocation':
        txn = AssertLocationV1.fromString(transaction); // transactions are only signed up until the maker's nonce limit
      ...
      case 'assertLocationV2':
        txn = AssertLocationV2.fromString(transaction); // transactions are only signed up until the maker's nonce limit
    ...
    errorResponse(req, res, env === 'development' ? error.message : 'Internal error', 500, env === 'development' ? error.errors : []);
  }
};
export const sample = async (req, res) => {
  ...
  const signedTxn1 = await txn.sign({
    owner
  });
